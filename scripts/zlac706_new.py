...
#######################################################################################################
    def motor_set_PID(self):
        # stop motor before set PID
        a1 = 6
        m_speed = 0
        a4 = check_code(a1, m_speed)
        self.serial.write(struct.pack(">BhB", int(a1), int(m_speed), int(a4)))
        ...
        kp = self.kp_gain_
        a4 = check_code(a1, kp)
        self.serial.write(struct.pack(">BhB", int(a1), int(kp), int(a4)))

        #set Ki
        a1 = 65
        ki = self.ki_gain_
        a4 = check_code(a1, ki)
        self.serial.write(struct.pack(">BhB", int(a1), int(ki), int(a4)))

        #set Kd
        a1 = 66
        kd = self.kd_gain_
        a4 = check_code(a1, kd)
        self.serial.write(struct.pack(">BhB", int(a1), int(kd), int(a4)))
    ...
    def set_torque_mode(self):
        ...
        #set Torque mode
        self.serial.write(b'\x02\x00\xC1\xC3')
        #set Torque = 0
        self.serial.write(b'\x08\x00\x00\x08')
        #start Motor
